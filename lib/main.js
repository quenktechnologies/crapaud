...
const runTest = (conf) => future_1.doFuture(function* () {
    ...
    if (conf.transform)
        script = yield execTransformScript(resolve(conf.transform, path.dirname(scriptPath)), scriptPath, script);
...
const getDriver = (browser) => future_1.liftP(() => new selenium_webdriver_1.Builder()
    .forBrowser(browser)
    .build());
const execTransformScript = (cliPath, jsPath, jsTxt) => future_1.fromCallback(cb => {
